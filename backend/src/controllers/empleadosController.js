...
async function agregarEmpleado(req, res) {   // alias de registrarEmpleado
  const {
    nombre,
    apellido,
    cargo,
...
async function actualizarEmpleado(req, res) {
  ...
  try {
    ...
    const request = pool.request();
    request.input("id", id);
    // (idem al insertar, con UPDATE...)
    const result = await request.query(`
      UPDATE Empleados SET
        Nombre=@nombre, Apellido=@apellido, Cargo=@cargo, EmpresaID=@empresaID,
        TipoDocumento=@tipoDocumento, NumeroDocumento=@numeroDocumento,
        FechaNacimiento=@fechaNacimiento, Edad=@edad, Genero=@genero,
        Raza=@raza, EstadoCivil=@estadoCivil, Municipio=@municipio,
        Direccion=@direccion, MedioTransporte=@medioTransporte,
